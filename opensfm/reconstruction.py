...
def bundle(
    ...
    report = pysfm.BAHelpers.bundle(
        ...
        0 if not use_subset else grid_size,
        config,
    )
...
def grow_reconstruction(
    ...
    while True:
        ...
        for image, resect_points in candidates:
            ...
            ratio_existing = len(resect_points) / len(new_tracks) if new_tracks else 1.0
            ...
            if should_retriangulate.should():
                ...
                align_reconstruction(reconstruction, gcp, config)
                b1rep = bundle(
                    reconstruction, camera_priors, rig_camera_priors, None, 8, config
                )
    ...
    bundle(reconstruction, camera_priors, rig_camera_priors, gcp, 32, config)
    remove_outliers(reconstruction, config)
    paint_reconstruction(data, tracks_manager, reconstruction)
    return reconstruction, report
